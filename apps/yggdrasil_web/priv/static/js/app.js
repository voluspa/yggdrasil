...
window.app = {
  start: function start() {
    ...
    _socket2.default.init(function (err) {
      if (err) return alert(err.message);

      var $cmd = $("#command");
      var $events = $("#events");

      _socket2.default.on("event", function (payload) {
      ...
      $cmd.on("keypress", function (e) {
        ...
        var cmdText = $cmd.val();
        _socket2.default.push("player_cmd", { text: cmdText });
        $cmd.val("");
      });

      _socket2.default.join();
    });
  }
};
...
var socket = null; // NOTE: The contents of this file will only be executed if
// you uncomment its entry in "web/static/js/app.js".

// To use Phoenix channels, the first step is to import Socket
// and connect at the socket path in "lib/my_app/endpoint.ex":
var game_channel = null;
var player_channel = null;
...
exports.default = {
  init: function init(done) {
    ...
    game_channel.join().receive("ok", function (resp) {
      player_channel = socket.channel(resp.topic, { "token": window.userToken });
      player_channel.join().receive("ok", function (resp) {
        done(null, resp);
